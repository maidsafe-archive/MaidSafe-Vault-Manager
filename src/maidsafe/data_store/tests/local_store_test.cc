...
TEST_CASE_METHOD(LocalStoreTest, "SuccessfulStore", "[Private][Behavioural]") {
  ...
  local_store_.PutVersion<MutableData>(dir_name, default_version, version0);
  local_store_.PutVersion<MutableData>(dir_name, version0, version1);
  local_store_.PutVersion<MutableData>(dir_name, version1, version2);

  auto retrieved_versions(local_store_.GetVersions<MutableData>(dir_name).get());
  REQUIRE(1U == retrieved_versions.size());
  REQUIRE(version2 == retrieved_versions.front());

  retrieved_versions = local_store_.GetBranch<MutableData>(dir_name, version2).get();
  ...
  local_store_.DeleteBranchUntilFork<MutableData>(dir_name, version2);
  retrieved_versions = local_store_.GetVersions<MutableData>(dir_name).get();
