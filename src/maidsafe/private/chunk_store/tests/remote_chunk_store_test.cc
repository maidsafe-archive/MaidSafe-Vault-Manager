...
class RemoteChunkStoreTest: public testing::Test {
 ...
 protected:
  void SetUp() {
    ...
    *signature = asymm::Sign(asymm::PlainText(signed_data_.data()), fob_.PrivateKey()).string();
...
TEST_F(RemoteChunkStoreTest, BEH_GetAndLock) {
  ...
  // existing MBO chunk
  GenerateChunk(ChunkType::kModifiableByOwner, 123, fob_.PrivateKey(), &name, &content);
  ASSERT_TRUE(this->chunk_store_->Store(name, content, store_success_callback_, fob_));
  EXPECT_EQ(kSuccess,
            this->chunk_store_->GetAndLock(name, ChunkVersion(), fob_, &retrieved_content));
  EXPECT_EQ(content.string(), retrieved_content);
...
TEST_F(RemoteChunkStoreTest, BEH_Modify) {
  ...
  {
    GenerateChunk(ChunkType::kDefault, 123, fob_.PrivateKey(), &name, &content);
    GenerateChunk(ChunkType::kModifiableByOwner, 123, fob_.PrivateKey(), &dummy, &new_content);
...
//  FLAGS_ms_logging_private = google::INFO;
  ...
  while (chunks.size() < kNumChunks) {
    ...
    ChunkId chunk_name;
    GenerateChunk(ChunkType::kModifiableByOwner, 123, fob_.PrivateKey(), &chunk_name,
                  &chunk_content);
    GenerateChunk(ChunkType::kModifiableByOwner, 123, fob_.PrivateKey(), &dummy,
                  &chunk_new_content);
...
TEST_F(RemoteChunkStoreTest, FUNC_MultiThreads) {
  const size_t kNumChunks(static_cast<size_t>(5));
  {
    ...
    while (chunks.size() < kNumChunks) {
      ...
      ChunkId chunk_name;
      GenerateChunk(ChunkType::kDefault, 123, fob_.PrivateKey(), &chunk_name, &chunk_content);
