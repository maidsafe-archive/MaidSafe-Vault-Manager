...
template <>
void ChunkStoreTest<ThreadsafeChunkStore>::InitChunkStore(std::shared_ptr<ChunkStore>* chunk_store,
                                                          const fs::path&,
...
class ThreadsafeChunkStoreTest : public testing::Test {
 public:
  ThreadsafeChunkStoreTest()
      : test_dir_(maidsafe::test::CreateTestPath("MaidSafe_TestThreadsafeChunkStore")),
        chunkname_(),
  ...
  void SetUp() {
    ...
    for (uint8_t i = 0; i < 30U; ++i)
      thread_group_.create_thread([this] { ThreadsafeChunkStoreTest::RunThread(); } );  // NOLINT (Fraser)
  }
  ...
  void StoreFromSourceFile(const size_t& num, const bool& check_flag) {
    for (uint16_t i = 1; i < num + 1; ++i) {
      ...
      ChunkId file_name(crypto::HashFile<crypto::SHA512>(path));
  ...
  void GetMemChunk(const ChunkId& chunk_name) {
    ...
    if (it != chunkname_.end()) {
      ...
      EXPECT_EQ((*it).string(), crypto::Hash<crypto::SHA512>(content).string());
    }
  }

  void GetFileChunk(const ChunkId& chunk_name, const fs::path& path) {
    ...
    if (it != chunkname_.end()) {
      EXPECT_EQ((*it), chunk_name);
      EXPECT_EQ((*it).string(), crypto::HashFile<crypto::SHA512>(path).string());
    }
  }

  void HasChunk(const ChunkId& chunk_name) {
    EXPECT_TRUE(threadsafe_chunk_store_->Has(chunk_name));
  }

  void DeleteChunk(const ChunkId& chunk_name) {
    EXPECT_TRUE(threadsafe_chunk_store_->Delete(chunk_name));
    EXPECT_FALSE(threadsafe_chunk_store_->Has(chunk_name));
  }

  void ChunkSize(const ChunkId& chunk_name, uint64_t* total_size) {
  ...
  void SetCapacity(const size_t& index, const std::vector<uint32_t>& capacity) {
    ...
    auto it = std::find(capacity.begin(), capacity.end(), threadsafe_chunk_store_->Capacity());
  ...
  void ChunkCount(const ChunkId& chunk_name) {
  ...
  void MoveChunk(const ChunkId& chunk_name, ChunkStore* another_chunk_store) {
    EXPECT_TRUE(threadsafe_chunk_store_->MoveTo(chunk_name, another_chunk_store));
    EXPECT_FALSE(threadsafe_chunk_store_->Has(chunk_name));
...
TEST_F(ThreadsafeChunkStoreTest, BEH_Misc) {
  ...
  for (size_t i = 0; i < delete_chunknames.size(); ++i) {
    functor f1 = std::bind(&ThreadsafeChunkStoreTest::DeleteChunk, this, delete_chunknames[i]);
  ...
  for (size_t i = 0; i < moveto_chunknames.size(); ++i) {
    functor f1 = std::bind(&ThreadsafeChunkStoreTest::MoveChunk, this, moveto_chunknames[i],
                           &another_chunk_store);
    functors.push_back(f1);
  }
  ...
  for (size_t i = 0; i < test_tscs::kIterationSize; ++i) {
    functor f1 = std::bind(&ThreadsafeChunkStoreTest::StoreContents, this, i, true);
    functor f2 = std::bind(&ThreadsafeChunkStoreTest::StoreFromSourceFile, this, i, true);
    functors.push_back(f1);
  ...
  for (size_t i = 0; i < chunkname_.size(); ++i) {
    functor f1 = std::bind(&ThreadsafeChunkStoreTest::HasChunk, this, chunkname_[i]);
    functors.push_back(f1);
  }
  ...
  for (size_t i = 0; i < chunkname_.size(); ++i) {
    std::string chunk(RandomAlphaNumericString(6));
    fs::path path(*test_dir_ / chunk);
    functor f1 = std::bind(&ThreadsafeChunkStoreTest::GetFileChunk, this, chunkname_[i], path);
    functor f2 = std::bind(&ThreadsafeChunkStoreTest::GetMemChunk, this, chunkname_[i]);
    functors.push_back(f1);
    functors.push_back(f2);
  }
  ...
  uint64_t total_size(0);
  for (size_t i = 0; i < chunkname_.size(); ++i) {
    functor f1 = std::bind(&ThreadsafeChunkStoreTest::ChunkSize, this, chunkname_[i], &total_size);
    functors.push_back(f1);
  }

  //  create Count functor
  for (size_t i = 0; i < chunkname_.size(); ++i) {
    functor f1 = std::bind(&ThreadsafeChunkStoreTest::ChunkCount, this, chunkname_[i]);
    functors.push_back(f1);
  }
  ...
  for (size_t i = 0; i < test_tscs::kIterationSize; ++i) {
    functor f1 = std::bind(&ThreadsafeChunkStoreTest::ChunkStoreCapacity, this, 0);
    functors.push_back(f1);
  }
