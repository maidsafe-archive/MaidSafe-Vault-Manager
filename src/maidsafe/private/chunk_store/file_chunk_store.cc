...
bool FileChunkStore::Init(const fs::path& storage_location, unsigned int dir_depth) {
  try {
    ...
    if (fs::exists(storage_location)) {
      ...
      if (!fs::create_directories(storage_location)) {
        LOG(kError) << "Failed to create storage location directory: " << storage_location;
        return false;
      }
      ResetChunkCount();
      ChunkStore::Clear();
...
std::string FileChunkStore::Get(const ChunkId& name) const {
  ...
  file_path.replace_extension("." + boost::lexical_cast<std::string>(ref_count));
...
bool FileChunkStore::Get(const ChunkId& name, const fs::path& sink_file_name) const {
  ...
  if (sink_file_name.empty()) {
    LOG(kError) << "Sink file path empty";
    return false;
  }
  ...
  source_file_path.replace_extension("." + boost::lexical_cast<std::string>(ref_count));

  boost::system::error_code ec;
  fs::copy_file(source_file_path, sink_file_name, fs::copy_option::overwrite_if_exists, ec);
  return !ec;
}
...
bool FileChunkStore::Store(const ChunkId& name, const std::string& content) {
  ...
  if (ref_count == 0) {
    ...
    old_path.replace_extension("." + boost::lexical_cast<std::string>(ref_count));
    ++ref_count;
    new_path.replace_extension("." + boost::lexical_cast<std::string>(ref_count));
  ...
  if (ref_count == 0) {
    ...
    try {
      ...
      if (delete_source_file)
        ...
        fs::copy_file(source_file_name, chunk_file, fs::copy_option::overwrite_if_exists);
...
bool FileChunkStore::Delete(const ChunkId& name) {
  ...
  if (ref_count == 1) {
    ...
    new_chunk_path.replace_extension("." + boost::lexical_cast<std::string>(ref_count));
  ...
  LOG(kError) << "End of function without positive return - name: " << Base32Substr(name);
  return false;
}
...
bool FileChunkStore::Modify(const ChunkId& name,
  ...
  uintmax_t content_size_difference;
  bool increase_size(false);
  if (!AssessSpaceRequirement(fs::file_size(chunk_file, ec2), fs::file_size(source_file_name, ec1),
                              &increase_size, &content_size_difference) || ec1 || ec2) {
    LOG(kError) << "Size differential unacceptable - increase_size: "
                << increase_size << ", name: " << Base32Substr(name)
  ...
  fs::copy_file(source_file_name, chunk_file, fs::copy_option::overwrite_if_exists, ec1);
  if (ec1) {
    LOG(kError) << "Failed to copy the file over - name: " << Base32Substr(name) << ", source: "
                << source_file_name << ", destination: " << chunk_file << ", result: "
                << ec1.value();
    return false;
  }
...
bool FileChunkStore::MoveTo(const ChunkId& name, ChunkStore* sink_chunk_store) {
  ...
  if (!sink_chunk_store) {
    LOG(kError) << "Chunk store passed is null: " << Base32Substr(name);
    return false;
  }
...
void FileChunkStore::Clear() {
  ...
  if (ec) {
    LOG(kWarning) << "Failed to remove " << storage_location_ << ": " << ec.message();
  }
  ChunkStore::Clear();
...
fs::path FileChunkStore::ChunkNameToFilePath(const ChunkId& name, bool generate_dirs) const {
  ...
  if (encoded_file_name.length() < dir_depth_for_chunk)
    dir_depth_for_chunk = static_cast<unsigned int>(encoded_file_name.length()) - 1;
...
uintmax_t FileChunkStore::GetChunkReferenceCount(const fs::path& chunk_path) const {
  boost::system::error_code ec;
  ...
  if (fs::exists(fs::path(chunk_path.string() + ".1"), ec)) {
    LOG(kInfo) << "Heuristic to prevent iteration for the most common case: " << chunk_path;
    return 1;
  }
  ...
  try {
    ...
    for (fs::directory_iterator it(chunk_path.parent_path());
      ...
      if (it->path().stem().string() == file_name && fs::is_regular_file(it->status()))
        return boost::lexical_cast<uintmax_t>(it->path().extension().string().substr(1));
    }
  }
  catch(const std::exception& e) {
