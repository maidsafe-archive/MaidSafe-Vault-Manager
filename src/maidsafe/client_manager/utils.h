...
#ifndef MAIDSAFE_CLIENT_MANAGER_UTILS_H_
#define MAIDSAFE_CLIENT_MANAGER_UTILS_H_
...
std::string GenerateVmidParameter(uint32_t process_index, uint16_t client_manager_port);

// Parses a --vmid argument of the PD vault into its constituent parts.
void ParseVmidParameter(const std::string& client_manager_identifier, uint32_t& process_index,
                        uint16_t& client_manager_port);
...
#ifdef TESTING
void SetTestEnvironmentVariables(Port test_client_manager_port,
...
#endif  // MAIDSAFE_CLIENT_MANAGER_UTILS_H_
